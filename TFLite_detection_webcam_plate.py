...
ACCESS_TOKEN = '3xdvj1gmB0Q4HNlabXdZ'



# Set up MQTT client and connect to Thingsboard
...
while True:
    ...
    for i in range(len(scores)):
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            plate = frame[ymin:ymax, xmin:xmax].copy()
            ...
            if plate.any():
                ...
                lpText = pytesseract.image_to_string(plate, config=options)

                lpText = lpText.replace("\n", "")

                text_width, text_height = cv2.getTextSize(lpText, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)

                cv2.putText(frame, lpText, (xmin, ymax + text_height + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)

                #if re.match(pattern1, lpText) or re.match(pattern2, lpText):

                client.publish('v1/devices/me/telemetry', json.dumps({'message': lpText}), 1)
